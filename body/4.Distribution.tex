...
\end{proposition}
\begin{proof}
    \[ M_{X_1+\cdots+X_k}(t)=\prod_{i=1}^k(p e^t + 1-p)^{n_i}=(p e^t + 1-p)^{\sum_{i=1}^kn_i} \]
\end{proof}
...
\begin{description}
    ...
    \item[概率质量函数] $p(x)=\frac1{b-a}\1{\{ a < x < b \}}(x)$
...
\begin{definition}[伽玛分布]
    若随机变量$X$的密度函数为
    \[ p(x) = \frac{\lambda^\alpha}{\Gamma(\alpha)}x^{\alpha-1}\ee^{-\lambda x}\1{\{ x \ge 0 \}}(x) \]
    则称$X$服从\textbf{伽玛分布}，记作$X \sim \Gamma(\alpha,\lambda)$，其中称$\alpha>0$为\underline{形状参数}，$\lambda>0$为\underline{尺度参数}。
\end{definition}
...
\begin{figure}[!ht]
    ...
    \caption{$\lambda$固定、不同$\alpha$的伽玛密度曲线}\label{fig:gamma_pdf}
\end{figure}

从图\ref{fig:gamma_pdf}中可以看出：
\begin{itemize}
...
\begin{description}
    ...
    \item[特征函数] $\phi_X(t)=\exp (i \mu t - \sigma |t|)$（见例\ref{ex:Cauchy_dist_chf}）
    \item[均值] 不存在
    \item[方差] 不存在
\end{description}
\begin{remark}
...
该密度函数的图像是一个只取非负值的偏态分布,见图~\ref{fig:chi_square_pdf}。
\begin{figure}[!ht]
    \centering
    ...
    \end{tikzpicture}
    \caption{$\chi^2(n)$分布的密度函数}\label{fig:chi_square_pdf}
\end{figure}

TODO
\begin{proposition}[卡方分布与gamma分布的联系]\label{prop:chi_square_gamma}
    若 $X \sim \chi^2(n)$，则$\chi^2(n) \sim \Gamma(n/2,1/2)$
\end{proposition}
\begin{proof}
    \begin{align*}
        f_{X^2}(y)&=\frac1{2\sqrt{y}}[f_X(\sqrt{y})+f_X(-\sqrt{y})]\\
                &=2\frac1{2\sqrt{y}}\frac1{\sqrt{2\pi}} \ee^{-\frac{y}{2}}\\
                &=(\frac{1}{2})^{\frac{1}{2}-1} / \Gamma(\frac{1}{2}) y^{\frac{1}{2}-1} \ee^{-\frac{1}{2}y}
    \end{align*}
\end{proof}
...
\end{corollary}
\begin{proof}
    由命题\ref{prop:sum_of_Gamma}，即Gamma分布的可加性易得
\end{proof}

\begin{corollary}
    若随机变量 $X \sim \Exp(1)$，则 $2X \sim \chi^2(2)$
\end{corollary}
\begin{proof}
    由命题\ref{prop:sum_of_Gamma}，即Gamma分布的可加性易得；与命题\ref{prop:times_of_Gamma}Gamma分布的数乘性易得。
\end{proof}
...
\begin{figure}[H]
    \centering
    \begin{tikzpicture}
        \node[box] (binom) at(4,4) {$B(n,p)$};
        \node[box] (binom1) at(8,4) {$B(1,p)$};
        \node[box] (Poisson) at(0,4) {$P(\lambda)$};
        \node[box] (Hypergeometric) at(4,8) {$h(n,N,M)$};
        \node[box] (negativeBi) at(4,0) {$NB(n,p)$};
        \node[box] (geometric) at(8,0) {$G(p)$};
        \draw[->] (binom1)--(binom);
        \node at(6.2,4.3) {$\sum X_i$};
        \draw[->] (Poisson)--(binom);
        \node at(2,4.3) {$X|\sum X_i$};
        \draw[->] (geometric)--(negativeBi);
        \node at(6.2,0.3) {$\sum X_i$};
        \draw[->,densely dotted] (3.5,4.4) arc(20:160:1.9);
        \node at(2,6) {$\lambda=np,n \to \infty$};
        \draw[->] (4.5,4.4) arc(160:20:1.9);
        \node at(6.2,6) {$n=1$};
        \draw[->] (4.8,3.5) arc(45:-225:1);
        \node at(4,2.4) {$\sum X_i$};
        \draw[->,densely dotted] (Hypergeometric)--(binom);
        \node at(4,6.8) {$p=\frac{M}{N},\frac{n}{N} \to 0$};
        \draw[->,densely dotted] (3.1,0) arc(260:180:3.6);
        \node at(1.5,1.5) {$\lambda=n(1-p),n \to \infty$};
        \draw[->] (4,-0.3) arc(-160:-20:2);
        \node at(6,-1) {$n=1$};
        \draw[->] (8,-0.42) arc(-150:150:0.8);
        \node at(8.2,1) {$min(X_i)$};
    \end{tikzpicture}
    \caption{离散分布间的关系}
\end{figure}
